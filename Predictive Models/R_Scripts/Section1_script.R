...
attach(Boston)
n = dim(Boston)[1] ##dim stands for dimensions

plot(lstat,medv)

train = data.frame(lstat,medv) ##makes a data frame with those two columns
test = data.frame(lstat,medv)
ind = order(test[,1]) ##index that is ordered by first column(lstat) in ascending order
test =test[ind,] ##still confused by this

MSE = NULL

kk = c(2,10,50,100,150,200,250,300,400,505) ##Vector

for(i in kk){
  
  near = kknn(medv~lstat,train,test,k=i,kernel = "rectangular") ##rectangular is for how kknn picks the closest neighbors, draws box around point
  aux = mean((test[,2]-near$fitted)^2) ##temp variable storing MSE
  
  MSE = c(MSE,aux)
  
  plot(lstat,medv,main=paste("k=",i),pch=19,cex=0.8,col="darkgray") ##paste concatenates the text together
  lines(test[,1],near$fitted,col=2,lwd=2)
  cat ("Press [enter] to continue") ##cat puts text on the console
  line <- readline() ##pauses execution until user enters something
}
...
for(i in seq(1,505,by=100)){ ##takes start value(1) and goes up by 100 until 505; 1, 101, 201, 301, etc..
  ii = near$C[i,1:20] ##indices of 20 closest points
  plot(lstat,medv,main=paste("k=",20),pch=19,cex=0.8,col="darkgray")
  lines(test[,1],near$fitted,col=2,lwd=2) ##plots the fitted values line
  abline(v=test[i,1],col=2,lty=2) ##plots a vertical line
  points(lstat[ii],medv[ii],pch=19,col="blue") ##colors the circles blue that closet
...
tr = sample(1:506,400) ##takes 400 points without replacement from 1-506

train = train[tr,]
test = test[-tr,] ##negative index removes it from the set
...
mMSE = apply(out_MSE,2,mean) ##apply takes a matrix object and takes the mean along the 2nd column
...
#################################
kcv = 10
n0 = round(n/kcv,0) ##second argument specifies number of decimal points
...
for(j in 1:kcv){
  ...
  if(n0<length(set)){val = sample(set,n0)} ##length gives the number of objects in the vector
...
hist(NBA$spread[favwin==0], add=TRUE, col=6) ##add makes the histogram appear on top of the other
